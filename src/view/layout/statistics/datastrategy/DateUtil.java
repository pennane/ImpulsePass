...
public class DateUtil {
	public static ZonedDateTime dateToZonedDateTime(Date date) {
		return ZonedDateTime.ofInstant(date.toInstant(), ZoneId.of("Z"));
	}

	public static ZonedDateTime toStartOfMonth(ZonedDateTime date) {
		return ZonedDateTime.of(date.getYear(), date.getMonth().getValue(), 1, 0, 0, 0, 0, ZoneId.of("Z"));
	}

	public static ZonedDateTime toStartOfWeek(ZonedDateTime date) {
		var day = (int) (Math.min(Math.floor(date.getDayOfMonth() / 7) * 7 + 1, 22));
		return ZonedDateTime.of(date.getYear(), date.getMonth().getValue(), day, 0, 0, 0, 0, ZoneId.of("Z"));
	}
